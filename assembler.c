...
void printM(OS *D, int c){ //---------------------Disassemble, print memory and registers--------------------------------------
  ...
  puts("");
  printf("Disassembly: ");
    printf("%02X  %02X  %02X: ",(unsigned char)*(D->E-3), (unsigned char)*(D->E-2), (unsigned char)*(D->E-1));                                          
...
{
  int k = 192 , SP = 191; // E go from 0xC0 to 0xFF   ----  SP go from 0xBF to 0x00
